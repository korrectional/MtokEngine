...
void AstroBelt_gameLoop() {
    if (playing) {
        ...
        if (AstroBelt_r1move) AstroBelt_rock1->move(0, 0, 0.2 * (1.0f + AstroBelt_score * 0.02));
        if (AstroBelt_r2move) AstroBelt_rock2->move(0, 0, 0.2 * (1.0f + AstroBelt_score * 0.02)); else if (AstroBelt_rock1->position.z > -15) AstroBelt_r2move = true;
        if (AstroBelt_r3move) AstroBelt_rock3->move(0, 0, 0.2 * (1.0f + AstroBelt_score * 0.02)); else if (AstroBelt_rock2->position.z > -15) AstroBelt_r3move = true;

        for (int rock = 0; rock < 3; rock++) {
            if (AstroBelt_rocks[rock]->position.z > 0) {
                if(rock==2){AstroBelt_rocks[rock]->setPosition((AstroBelt_player->position.x) - 4.0, -1.5, -50);}else{
...
void AstroBelt_showOverlay() {
    ...
    Write(2, 1, 1, std::to_string(AstroBelt_score).c_str());
}
